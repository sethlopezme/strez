...
impl Settings {
     ...
     */
    pub fn from_clap<'a>(args: &'a clap::ArgMatches) -> Result<Settings> {
        Ok(Settings {
            ...
            platform: args.values_of("platform")
                          .map_or(Ok(vec![PlatformKind::Android, PlatformKind::Ios]), |platforms| {
                              platforms.map(|platform| platform.parse::<PlatformKind>())
                                       .collect::<Result<Vec<PlatformKind>>>()
                          })?
        })
    }
}
 ...
 */
fn str_to_pathbuf(path_str: &str, is_file: bool) -> Result<PathBuf> {
    let pathbuf = fs::canonicalize(path_str).map_err(|e| format!("{} \"{}\"", e.description(), path_str))?;

    if is_file && !pathbuf.is_file() {
        Err(format!("invalid file \"{}\"", path_str))
    } else if !is_file && !pathbuf.is_dir() {
        Err(format!("invalid directory \"{}\"", path_str))
    } else {
        Ok(pathbuf)
    }
}
